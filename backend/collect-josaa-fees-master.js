...
const READY =
  "./josaa-fee-master-ready.json";

const REVIEW =
  "./josaa-fee-master-review.json";

const REPORT =
  "./josaa-fee-master-report.json";
...
function loadAlreadyAttemptedColleges() {

  const processed =
    new Set();

  /*
    Scan every previous batch/retry result file.

    Examples:
      josaa-fee-batch-04-ready.json
      josaa-fee-batch-05-review.json
      josaa-fee-batch-06-safe.json
      josaa-fee-retry-01-ready.json
      josaa-fee-retry-01-needs-review.json

    Master's own output files are excluded.
  */

  const patterns = [
    /^josaa-fee-batch-\d+-(ready|safe|review|needs-review|report)\.json$/i,
    /^josaa-fee-retry-\d+(?:-[a-z0-9-]+)?-(ready|safe|review|needs-review|report)\.json$/i
  ];
  ...
  console.log(
    "Previous result files detected:",
    files.length
  );
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(
        `Skipping unreadable history file: ${file}`
      );
...
if (!Array.isArray(priorityQueue)) {
  throw new Error(
...
console.log(
  "Already attempted identity keys:",
  alreadyAttempted.size
);
...
if (
  ...
  console.log(
    "No never-attempted NO_FEE_DATA colleges remain."
  );
}
