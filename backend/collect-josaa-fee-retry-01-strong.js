...
const READY =
  "./josaa-fee-retry-01-strong-ready.json";

const REVIEW =
  "./josaa-fee-retry-01-strong-review.json";

const REPORT =
  "./josaa-fee-retry-01-strong-report.json";

const BATCH_SIZE = 25;
const TIMEOUT = 25000;

const CURRENT_BATCH = 8;

/*
  ============================================================
  DUPLICATE-SAFE BATCH SYSTEM
  ============================================================

  Batch-08 must never process a college already attempted in
  Batch-01 ... Batch-07.

  We check:
    1. college_id
    2. JoSAA/institute code
    3. normalized college name

  We also count READY, SAFE, REVIEW, NEEDS-REVIEW and REPORT
  files as already attempted.

  Current Batch-08 files are NOT considered previous files.
*/
...
function addCollegeIdentity(
  ...
  if (
    ...
    processed.add(
      `college_id:${normalizeCollegeKey(
        collegeId
      )}`
    );
  }
...
function loadAlreadyProcessedColleges() {
  ...
  /*
    ...
    josaa-fee-batch-04-safe.json
    josaa-fee-batch-05-review.json
    josaa-fee-batch-05-needs-review.json
    josaa-fee-batch-06-report.json
  */

  const batchFilePattern =
    /^josaa-fee-batch-(\d+)-(ready|safe|review|needs-review|report)\.json$/i;
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.log(
        `Skipping unreadable previous batch file: ${file.name}`
      );
...
async function researchOfficial(college) {
  const queries = [
    `"${college.josaa_name}" "fee structure" B.Tech`,
    `"${college.josaa_name}" "B.Tech fee structure"`,
    `"${college.josaa_name}" "semester fee" B.Tech`,
    `"${college.josaa_name}" "academic fee" B.Tech`,
    `"${college.josaa_name}" "tuition fee" B.Tech`,
    `"${college.josaa_name}" fee pdf B.Tech`,
    `"${college.josaa_name}" admission brochure fee`,
    `"${college.josaa_name}" fee circular B.Tech`
  ];
...
async function researchOpenWeb(
  ...
  const queries = [
    ...
    `"${college.josaa_name}" "B.Tech fee structure"`,
    `"${college.josaa_name}" "semester fee"`,
    `"${college.josaa_name}" fee pdf`,
    `"${college.josaa_name}" admission brochure fee`
  ];
...
async function researchSecondary(
  ...
  for (
    ...
    const query =
      `"${college.josaa_name}" "B.Tech fees" "fee structure" site:${source.domain}`;
...
if (
  ...
  throw new Error(
    "Priority queue JSON must contain an array"
  );
}

/*
  Get colleges already attempted in
  Batch-01 through Batch-07.
*/

const alreadyProcessed =
  loadAlreadyProcessedColleges();

/*
  Current unresolved JoSAA colleges.
*/

const noFeeCandidates =
...
/*
  ...
  Do NOT use fixed offsets such as:

  slice(100, 125)

  because the priority queue changes after imports.

  Instead:
    - start from current NO_FEE_DATA queue
    - remove colleges already attempted
    - select next 25 unique colleges
*/

const freshCandidates =
  noFeeCandidates.filter(
    college =>
      !wasAlreadyProcessed(
        college,
        alreadyProcessed
...
console.log(
  "Previously processed identity keys:",
  alreadyProcessed.size
);

console.log(
  "Current NO_FEE_DATA candidates:",
  noFeeCandidates.length
);

console.log(
  "Retry candidates available:",
  freshCandidates.length
);

console.log(
  "New colleges selected:",
  queue.length
);
...
if (
  ...
  console.log(
    "No new unprocessed NO_FEE_DATA colleges remain."
  );
}
