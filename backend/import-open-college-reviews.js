  ...
  async (req, res, next) => {
    ...
    try {
      ...
      */
      ...
      | IMPORTANT:
      |
      | college_sentiment_summary is LEFT JOINED.
      |
      | Therefore:
      |
      | - colleges WITH reviews get reviewScore
      | - colleges WITHOUT reviews still appear
      | - missing reviewScore stays NULL
      |
      | Frontend must NOT convert NULL reviewScore to 0.
      |--------------------------------------------------------------------------
      */
      ...
      let query = `
        ...
        |--------------------------------------------------------------------------
        |
        | college_sentiment_summary.college_id is VARCHAR/TEXT.
        | Review pipeline stores the same college slug/id.
        |
        | LEFT JOIN prevents colleges without review data
        | from disappearing from counselling results.
        |--------------------------------------------------------------------------
        */
      ...
      */
      ...
      |--------------------------------------------------------------------------
      |
      | PostgreSQL NUMERIC values can arrive through pg as strings.
      | Convert them to numbers before sending to frontend.
      |
      | IMPORTANT:
      | Missing review values remain NULL.
      |--------------------------------------------------------------------------
      */
