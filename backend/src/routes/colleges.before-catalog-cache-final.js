...
| - response structure
| - state/type/search filtering
|
| Only the complete unfiltered /api/colleges response is cached.
|
*/
...
router.get(
  ...
  async (req, res, next) => {
    ...
    try {
      ...
      |--------------------------------------------------------------------------
      |
      | The frontend catalog request normally calls:
      |
      | GET /api/colleges
      |
      | without state/type/q.
      |
      | Only that complete catalog is cached.
      |
      | Filtered requests continue through the original SQL path.
      |
      */
      ...
      */
      ...
      if (
        ...
        collegesCatalogCache.expiresAt >
          Date.now()
      ) {

        res.set(
          "X-TruMarg-College-Cache",
          "HIT"
        );
      ...
      |--------------------------------------------------------------------------
      |
      | Important for load spikes.
      |
      | If 100 users hit /api/colleges while cache is empty,
      | PostgreSQL should NOT receive 100 copies of the expensive query.
      |
      | The first request starts the SQL query.
      | Other requests await the same Promise.
      |
      */
      ...
      |--------------------------------------------------------------------------
      |
      | Everything inside this function preserves the original route logic.
      |
      */
      ...
      */
      ...
      |--------------------------------------------------------------------------
      |
      | state/type/q requests retain the exact original behavior.
      |
      */
      ...
      |--------------------------------------------------------------------------
      |
      | Never leave an unresolved in-flight Promise reference after failure.
      |
      */
